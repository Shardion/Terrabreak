...
public class TheGarbageSortingSquad : IRelic<RelicState>
{
    public string Name => "The Garbage-Sorting Squad";
    public string Description => "";
    public IRelicSeries Series => Registries.RelicSeries.Forward["InvestorCore"];
    public string EffectDescription => "Friendly monsters prioritise attacking enemy Rodent-class monsters.";
        ...
        RelicState thisState)
    {
        ...
        BattleMonster? enemyRodentMonster = invocation.DefendingPlayer.GetMonsterEnumerator()
            .Shuffle()
            .FirstOrDefault(monster => monster.Monster.Classification == MonsterClassification.Rodent
                                       && !BattleRules.CheckKnockout(monster));
        if (enemyRodentMonster is not null)
        {
            return new([], enemyRodentMonster);
        }
